window.onload = function () {
  ...
  */
  ...
  function onMouseDown(e) {
    ...
    if (check_edit_area()) {
      if (e.shiftKey && !cur_info['down']
        ...
        console.info( 'star cur' );
        start_cur();
      } else {
        console.info( 'change bw' );
        change_bw();
        change_bw_flag = true;
      }
    } else {
    ...
    if (check_clip_area()) {
      if (e.shiftKey) {
        if (cur_info['c_down'] && !cur_info['c_rect'] && !cur_info['down'] && !cur_info['rect']) {
          ...
          if ((cur_info['view_w'] % 8) > 0) {
            cur_info['c_view_w']++;
          }
          if ((cur_info['view_h'] % 8) > 0) {
            cur_info['c_view_h']++;
          }
        }
      }
  ...
  console.info( 'eventos de raton...', cursor );
  ...
  file.onchange = function () {
    ...
    reader.onload = function () {
      ...
      loadUserDataIntoEditor(up_array);
    }
  }
};
